...
pub enum ZoomMode {
    ...
    /// 用户手动缩放出来的倍率。
    Free,
    /// **还没定**：刚打开一张图，等待 [`ViewTransform::initial`] 的结果。
    ///
    /// 选 1:1 还是适应窗口，要看「画布尺寸 + 屏幕缩放」，而这两样在交付打开结果的
    /// 那一刻都拿不到（冷启动时窗口还没绘制过，画布尺寸是 0）。这个变体就是那段
    /// 空档的占位：绘制层按 [`ViewTransform::initial`] 就地求值（与「适应窗口」同一
    /// 套机制，见 `render/viewport.rs`），视图层拿到画布尺寸后再把它定下来。
    ///
    /// 之所以要显式建模，而不是先用 1:1 顶着：1:1 的倍率是 `1 / pixel_ratio`，
    /// 硬写一个 1.0 在高分屏上就不是 1:1 —— 第一帧会按错误倍率画出来，
    /// 下一帧再跳回去，而这种「头一帧偏一点」的错几乎没人会去查。
    Pending,
}
...
impl ViewTransform {
    ...
    /// 「初始视图还没定」的占位变换。
    ///
    /// 只用在「打开结果已交付、但画布还没量到尺寸」的那段空档（见 [`ZoomMode::Pending`]）。
    /// 绘制层对它就地求值、视图层拿到画布尺寸后用 [`ViewTransform::initial`] 定下来 ——
    /// 两处调用同一个函数，所以画面从第一帧起就是最终的样子：不会先按一种模式画出来、
    /// 下一帧再跳成另一种。
    pub fn pending() -> Self {
        Self {
            // 倍率不会被真正使用（绘制层会换成就地求值的结果）。取 1.0 是为了
            // 万一有别的路径读它，读到的也是「100%」而不是 0 或 NaN。
            scale: 1.0,
            pan: Vec2::ZERO,
    ...
    /// [`ZoomMode::Pending`] 也保持不动 —— 它马上就会被定义成适应或 1:1，
    /// 此刻改它只会凭空制造一次跳变。
